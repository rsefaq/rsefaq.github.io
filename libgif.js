/*SuperGif
  Git: https://github.com/buzzfeed/libgif-js
  Last Commits: 2015-03-27
  <Change Log>
  - 2023-05-17: progress bar 수정
*/
(function(root, factory) {
  ...
  var byteToBitArr = function(bite) {
      var a = [];
      for (var i = 7; i >= 0; i--) a.push(!!(bite & (1 << i)));
      return a;
    };
  var Stream = function(data) {
      ...
      this.readBytes = function(n) {
          var bytes = [];
          for (var i = 0; i < n; i++) bytes.push(this.readByte());
          return bytes;
        };
      this.read = function(n) {
          var s = '';
          for (var i = 0; i < n; i++) s += String.fromCharCode(this.readByte());
          return s;
        };
  ...
  var parseGIF = function(st, handler) {
      ...
      var parseCT = function(entries) {
          var ct = [];
          for (var i = 0; i < entries; i++) ct.push(st.readBytes(3));
          return ct;
        };
      ...
      var parseImg = function(img) {
          ...
          if (img.interlaced) img.pixels = deinterlace(img.pixels, img.width);
  ...
  var SuperGif = function(opts) {
      var options = { vp_l: 0, vp_t: 0, vp_w: null, vp_h: null, c_w: null, c_h: null };
      ...
      var canvasScale = get_canvas_scale();
      var clear = function() {
          transparency = null;
          delay = null;
      ...
      var setFrameOffset = function(frame, offset) {
          ...
          if (typeof offset.y !== 'undefined') frameOffsets[frame].y = offset.y;
        };
      var doShowProgress = function(pos, length, draw) {
          if (draw && showProgressBar) {
            ...
            if (options.is_vp) {
              ...
              top = (canvas.height - height) * (ctx_scaled ? canvasScale : 1);
              mid = ((pos / length) * canvas.width) * (ctx_scaled ? canvasScale : 1);
              width = canvas.width * (ctx_scaled ? canvasScale : 1);
              height *= ctx_scaled ? canvasScale : 1;
            }
      ...
      var player = (function() {
          ...
          var putFrame = function() {
              ...
              ctx.drawImage(tmpCanvas, 0, 0);
              if (showProgressBar && (stream !== undefined) && (stream.data.length > 0)) {
                ctx.globalCompositeOperation = "source-over";
                doShowProgress(i, frames.length-1, showProgressBar);
              }
            };
          var play = function() {
              playing = true;
          ...
          return {
              init: function() {
                if (loadError) return;
                if (!(options.c_w && options.c_h)) ctx.scale(canvasScale, canvasScale);
      ...
      var handler = {
          ...
          eof: function(block) {
            ...
            if (!(options.c_w && options.c_h)) {
              canvas.width = hdr.width * canvasScale;
              canvas.height = hdr.height * canvasScale;
            }
            player.init();
            loading = false;
      ...
      return {
          ...
          load_url: function(src, callback) {
            ...
            h.onload = function(e) {
                if (this.status != 200) doLoadError('xhr - response');
                if (!('response' in this)) this.response = new VBArray(this.responseText).toArray().map(String.fromCharCode).join('');
                var data = this.response;
                if (data.toString().indexOf("ArrayBuffer") > 0) data = new Uint8Array(data);
                stream = new Stream(data);
                setTimeout(doParse, 0);
              };
            h.onprogress = function(e) {
                if (e.lengthComputable) doShowProgress(e.loaded, e.total, showProgressBar);
              };
            h.onerror = function() {
                doLoadError('xhr');
              };
